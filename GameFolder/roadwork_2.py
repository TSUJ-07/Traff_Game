...
pygame.display.set_caption("Car Game/ TRAFF_GAME/ ANIME X TRAFF") # <-- WHAT THE HELL ARE WE DOING WITH THIS???? -JW
...
def user_movement():
    ...
    while True:
        ...
        if collision(traffic_obj.user_car_rect, traffic_obj.obstacles):
            config_2.MP3["crash"].play(0)
            # pygame.time.delay(5)
            Utilities.display_window(screen)  # Display cover
            config_2.MP3["failure"].play()
            Utilities.prep_text(f"Your Score is: {score}", 28, (255,255,0), y_scale=3)
            Utilities.failure()
            question= Utilities.failure()
            if question == "Restart":
                score= 0
                #Make system variable == True /// Or better yet, just call the function to restart game
                print() #This is the initial game method to call ----- {Currently Empty}
            else:
                pygame.quit()
                sys.exit(0)
